...
const DropDownMenu = ({ label, link, listComponent, navbarState, showMenu }) => {
	...
	return (
		...
		>
			<Flex
				className={`flex text-white font-poppins font-medium text-[15px] relative after:absolute after:w-full lg:after:h-[3px] after:h-[1px] lg:after:bg-white after:bg-[#92A2B8] lg:after:bottom-[-5px]
        after:bottom-[-13px] ${
			...
			>
				...
				<div className='lg:absolute w-3 h-3 left-1/2 top-0 -translate-x-1/2  -translate-y-1/2 bg-[#d9e3ec] rotate-45 lg:block hidden'></div>
				<List
					className={`lg:bg-[#d9e3ec] flex flex-col w-full lg:w-fit lg:px-6 pl-4 lg:rounded-md mt-5 lg:mt-0 lg:shadow-around-small ${
						open && showMenu ? "lg:border-none border-b-[1px] border-[#92A2B8]" : ""
					}`}
				>
					{listValues.map((value, index) => (
						<ListItem
							className={`font-poppins lg:font-semibold font-medium lg:text-light-blue py-4 lg:text-[15px] text-[13px] lg:border-t-0 ${
								!(index == listValues.length - 1) ? "border-b-[1px]" : ""
							} border-[#92A2B8] text-white lg:text-nowrap`}
							onClick={navbarState}
						>
							<Link
								to={`${listComponent[value]}`}
								onClick={() => {
									setOpen(false)
								}}
							>
